...
Our goal is to assess whether the data provide enough evidence to reject $H_0$. This helps connect the statistical test to our original question about which CTA performs better in practice.
...
```
 ...
 Our test statistic is $z \approx 2.14$, and the associated two-sided p-value is approximately $0.033$.Since this is well below the conventional significance level of $\alpha = 0.05$, we **reject the null hypothesis**. The data provide strong evidence that the two CTAs produce different sign-up rates. In practical terms, CTA A appears to outperform CTA B, and this difference is unlikely to be due to chance alone. While the result is statistically significant, it's still helpful to think about how meaningful this difference is in practice.
...
The results are striking. While a single test has a nominal false positive rate o f 5%, peeking at 10 intermediate points inflates this to roughly **19%**— more than five times higher. This means that if you follow your boss's strategy, more than 1 in 4 experiments will produce a false positive, leading you to declare a winner when there is truly no difference.
